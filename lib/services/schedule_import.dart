...
class ScheduleImport {
  ...
  Future<List<ScheduleEvent>?> pickAndParse() async {
    try {
      ...
      if (await file.length() > _maxFileSizeBytes) {
        throw Exception("File too large (>10MB)");
      }
  ...
  Future<ImportResult> commitImport(List<ScheduleEvent> events) async {
    try {
      if (events.isEmpty) {
        return ImportResult(success: false, message: "No classes to import");
      }
      ...
      for (var e in events) {
        final key =
            "${e.title}-${e.date}-${e.startTime}-${e.section?.trim().toLowerCase() ?? ""}";
      ...
      LogService.info(
          "Deduplication: ${events.length} -> ${deduplicatedList.length}");
      ...
      return ImportResult(
        ...
        section:
            deduplicatedList.isNotEmpty ? deduplicatedList.first.section : null,
      );
    } catch (e) {
      return ImportResult(
          success: false, message: "Import failed: ${e.toString()}");
    }
  }
  ...
  Future<List<ScheduleEvent>> _parseExcel(File file) async {
    ...
    final cutoff = await PreferencesService.getMorningCutoff();
    ...
    for (var entry in excel.tables.entries) {
      ...
      for (int c = 0; c < table.rows[headerRowIdx!].length; c++) {
        final val =
            table.rows[headerRowIdx][c]?.value?.toString().toLowerCase() ?? "";
      ...
      String? lastSeenDate;
      Set<int> rowsToSkip =
          {}; // Track rows that are just professor names for the row above
        ...
        for (var entry in timeSlots.entries) {
          ...
          if (title.contains('\n')) {
            ...
            if (r + 1 < table.rows.length) {
              ...
              final nextRowDateVal = dateColumnIdx < nextRow.length
                  ? nextRow[dateColumnIdx]?.value?.toString()
                  : null;

              // If date is empty OR matches current row's date exactly, it might be a sub-row (professor)
              final isDuplicateDate = (nextRowDateVal == null ||
                  nextRowDateVal.isEmpty ||
                  nextRowDateVal == "null" ||
                  nextRowDateVal == currentDateVal);
              ...
              if (isDuplicateDate && colIdx < nextRow.length) {
                ...
                if (profVal != null &&
                  ...
                  if (profLower.contains('prof') ||
                      profLower.contains('dr.') ||
                      profVal.split(' ').length >= 2) {
                    professor = profVal;
          ...
          try {
            ...
            if (times.length >= 2) {
              ...
              startTime = TimeHelpers.normalizeTime(startRaw, cutoff: cutoff);
              endTime = TimeHelpers.normalizeTime(endRaw, cutoff: cutoff);
            }
          } catch (e) {
            LogService.warning("Failed to parse time range: $timeRange");
            continue;
          }
          ...
          // Skip if it looks like a professor name or header
          if (titleLower.contains('prof.') ||
              titleLower.contains('dr.') ||
              titleLower.contains('professor') ||
              titleLower.contains('faculty') ||
              titleLower.startsWith('time') ||
              titleLower.contains('day/time')) {
            continue;
          }
  ...
  Future<List<ScheduleEvent>> _parseExcelLinearFallback(Sheet table) async {
    ...
    for (var row in table.rows.skip(1)) {
      ...
      if (titleStr.isEmpty ||
          titleStr == "null" ||
          titleStr.toLowerCase() == "refresh") {
        continue;
      }

      // Basic time validation
      if (!timeRegex.hasMatch(rawStart) || !timeRegex.hasMatch(rawEnd)) {
        continue;
      }

      final startTime = TimeHelpers.normalizeTime(rawStart, cutoff: cutoff);
      final endTime = TimeHelpers.normalizeTime(rawEnd, cutoff: cutoff);
